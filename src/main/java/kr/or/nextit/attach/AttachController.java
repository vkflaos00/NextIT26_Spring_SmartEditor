		...
		logger.info("AttachController attachDownload atchNo : "+ atchNo);
		 ...
		 if(file.isFile()) {
			 ...
			 /*	- resp.setContentType("application/octect-stream");
			 	- 웹서버는 브라우저로 전송될 페이지가 html인 경우 text/html을 표준 
			 	MIME타입으로 지정합니다.
			 		MIME타입을 변경 또는 인코딩셋을 변경하고자 할때, 
			 		setContentType메소드를 사용합니다.
			 	- octet-stream은 8비트 바이너리배열을 의미하며, 
			 	application이 지정되지 않았거나 형식을 모를때 사용합니다.
			 	- 브라우저는 기본적으로 자신이 알고 있는 타입일 경우 기본적으로
			 	 브라우저 자체에 출력하려고 시도합니다.
			 		따라서 임의의 형식으로 넣는 것입니다. 그래야 인식할 수 
			 		없는 mime-type이기 때문에 브라우저가 다운로드를 처리합니다.
			 		즉, 브라우저는 octet-stream으로 MIME타입이 지정된 경우 
			 		바이너리 데이터로만 다운로드가 가능하게됩니다.*/
			 resp.setHeader("Content-Disposition", "attachment; filename=\"" + originalName +"\"");
			 /*	- Content-Disposition는 응답 body에 해당 컨테츠에 대한 성향을 
			  * 	지정하는 속성이며
			 		주로 사용되는 기본 값은 inline으로 web에 전달되는 데이터 형태를 뜻한다.
			 		하지만 파일을 전달할 경우 attachment를 추가하여 filename을 함께 
			 		주면 body에 있는
